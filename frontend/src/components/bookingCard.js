...
const BookingCard = ({ numberOfTickets, showId, movie, _id, totalAmount }) => {
  ...
  return (
    <div className="flex max-w-md p-4 m-2 border-2 border-solid border-black rounded-lg hover:bg-slate-200">
      <img className="w-1/3 h-auto object-contain rounded-md" src={`${movieDetails.posterUrl}`} alt="Movie Poster" />
      <div className="ml-4">
        <h4 className="font-bold text-lg">{movieDetails.title}</h4>
        ...
        <br />
        <button className="mt-2 btn btn-info" onClick={downloadTicket}>
          Download Ticket
        </button>
      </div>
    </div>
